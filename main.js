...
let bgMusic;
let bgSky;    // Изображение неба
let bgRoad;   // Изображение дороги
let bgSkyInv, bgRoadInv; // Инвертированные изображения неба и дороги
let bgSkyX = 0;  // Позиция неба по X
let bgRoadX = 0; // Позиция дороги по X
...
function loadAssets(onComplete) {
    ...
    bgSky.onload = checkLoaded;
    // Загрузка инвертированного фона неба
    bgSkyInv = new Image();
    bgSkyInv.src = 'assets/bg_sky_inv.png';
    bgSkyInv.onload = checkLoaded;

    // Загрузка фона дороги
    bgRoad = new Image();
    bgRoad.src = 'assets/bg_road.png';
    bgRoad.onload = checkLoaded;
    // Загрузка инвертированного фона дороги
    bgRoadInv = new Image();
    bgRoadInv.src = 'assets/bg_road_inv.png';
    bgRoadInv.onload = checkLoaded;

    // Увеличиваем количество загружаемых ресурсов, так как теперь загружаем 2 фона вместо 1
    toLoad += 2;
...
function resetGame() {
    ...
    bgX1 = 0;
    bgX2 = canvas.width;
    // Инициализация позиций параллакс-фона
    bgSkyX = 0;
    bgRoadX = 0;
...
function drawBackground() {
    const skyX = Math.round(bgSkyX);
    if (!bgSky || !bgSkyInv || !bgRoad || !bgRoadInv) return;

    const canvasWidth = canvas.width;
    const canvasHeight = canvas.height;
    const roadY = canvasHeight * 0.45;
    const roadHeight = canvasHeight - roadY;
    ...
    // Зацикливание
    if (bgSkyX <= -canvasWidth * 2) bgSkyX += canvasWidth * 2;
    if (bgRoadX <= -canvasWidth * 2) bgRoadX += canvasWidth * 2;

    // Небо: [sky → skyInv → sky → skyInv]
    ctx.drawImage(bgSky,     0, 0, bgSky.width, bgSky.height,     skyX, 0, canvasWidth, roadY);
    ctx.drawImage(bgSkyInv,  0, 0, bgSkyInv.width, bgSkyInv.height, skyX + canvasWidth, 0, canvasWidth, roadY);
    ctx.drawImage(bgSky,     0, 0, bgSky.width, bgSky.height,     skyX + canvasWidth * 2, 0, canvasWidth, roadY);
    ctx.drawImage(bgSkyInv,  0, 0, bgSkyInv.width, bgSkyInv.height, skyX + canvasWidth * 3, 0, canvasWidth, roadY);

    // Округляем позицию дороги для плавности
    const roadX = Math.round(bgRoadX);
    // Дорога: [road → roadInv → road → roadInv]
    ctx.drawImage(bgRoad,     0, 0, bgRoad.width, bgRoad.height,     roadX,                 roadY, canvasWidth, roadHeight);
    ctx.drawImage(bgRoadInv,  0, 0, bgRoadInv.width, bgRoadInv.height, roadX + canvasWidth, roadY, canvasWidth, roadHeight);
    ctx.drawImage(bgRoad,     0, 0, bgRoad.width, bgRoad.height,     roadX + canvasWidth * 2, roadY, canvasWidth, roadHeight);
    ctx.drawImage(bgRoadInv,  0, 0, bgRoadInv.width, bgRoadInv.height, roadX + canvasWidth * 3, roadY, canvasWidth, roadHeight);
}
function initGame() {
    drawBackground(); // Сначала нарисуем наш новый фон (небо и дорогу)
    resetGame();
    gameStarted = true;
    if (bgMusic) {
        bgMusic.currentTime = 0;
...
/** Показывает 7 слайдов с assets/slide1.png–slide7.png и loading.mp3, затем вызывает onComplete() */
function startLoaderSlideshow(onComplete) {
  const slides = [
    'assets/slide1.png',
    'assets/slide2.png',
    'assets/slide3.png',
  ...
  let currentIndex = 0;
  const loadingAudio = new Audio('assets/loading.mp3');
  loadingAudio.play();
  const loaderDiv = document.getElementById('loaderSlideshow');
  const img = document.getElementById('loaderSlideImage');
  img.style.opacity = 0;
...
function draw() {
    ...
    // Draw background (single call, no mirroring needed)
    drawBackground();
        ...
        if (startBtn && startOverlay) {
            startBtn.addEventListener('click', () => {
                ...
                function maybeStartGame(){
                    if (!(assetsReady && slidesDone)) return;
                ...
                // Стартуем слайд‑шоу (slidesDone=true по завершении)
...
function spawnValera() {
    ...
    valeras.push({
        x: canvas.width,
        y: pitY - (canvas.height * 0.10),  // поднято на 10% высоты канваса
        width: runner.width,
        height: runner.height,
        audioPlayed: false
    });
...
function spawnCortage() {
    ...
    const carY = pitY - (canvas.height * 0.10);
